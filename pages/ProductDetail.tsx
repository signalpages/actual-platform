...
import { getAssetBySlug, runAudit } from '../lib/dataBridge';
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (data && auditProcessed.current !== slug) {
        if (shouldAutoRun || !storedAudits[slug.toLowerCase()]) {
    ...
    load();
  }, [slug, shouldAutoRun, handleDeepScan]);
  ...
  if (!asset) return (
    <div className="max-w-xl mx-auto px-6 py-20 text-center">
      <div className="bg-white border border-slate-200 p-12 rounded-3xl shadow-sm">
        ...
        <Link to="/" className="inline-block bg-slate-900 text-white px-8 py-3 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-blue-600 transition-all">Return Home</Link>
      </div>
    </div>
  );
